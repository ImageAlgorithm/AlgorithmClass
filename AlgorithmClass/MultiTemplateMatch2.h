...
#ifndef _MULTI_TEMPLATE_MATCH2_H
#define _MULTI_TEMPLATE_MATCH2_H
...
public:
	...
	MultiMatch2();
	MultiMatch2(int fileNum);
	~MultiMatch2();

	//************************************
	// Method:    GetData：读取识别需要的数据
	// FullName:  MultiMatch::GetData
	// Access:    public 
	// Returns:   bool
	// Qualifier:
	...
	//************************************
	bool InitData(Mat &InputImg, vector<string> files);
	...
	//************************************
	void threadproc(InputImagInfo info, TmpInfo tmpinfo, double dScoreThreshold);
...
public:
	...
	TmpInfo* tmpInfo; // 模板信息指针（指向一个与文件个数同样大小的TmpInfo数组
	InputImagInfo imageInfo; // 待识别图像信息
};
