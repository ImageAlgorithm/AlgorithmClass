...
//************************************
bool SingleMatch2::comp(MatchRst a, MatchRst b)
{
	return (a.dScore) > (b.dScore);
}

SingleMatch2::SingleMatch2()
{
	nMaxPyCnt = 2;
	nMinTmpSize = 20;
	nDispersion = 1;
...
bool SingleMatch2::CreateDoubleMatrix(double **&matrix, CvSize size)
{
...
bool SingleMatch2::DeleteDoubleMatrix(double **&matrix, CvSize size)
{
	if (size.width <= 0 || size.height <= 0)
	{
		return false;
...
bool SingleMatch2::GetSobelEdge(Mat InputImg, Mat &InputSobelDx, Mat &InputSobelDy)
{
	if (InputImg.empty())
	{
		return false;
...
int SingleMatch2::pyr(int nTmpHeight, int nTmpWidth)      //降采样次数
{
...
bool SingleMatch2::CreateMatchEdgeModel(Mat TemplateSobelXImg, Mat TemplateSobelYImg, double maxThre, double minThre, int &nofEdgePiex,
...
float SingleMatch2::InvSqrt(float x)
{
	float xhalf = 0.5f*x;
	int i = *(int*)&x;
	i = 0x5f3759df - (i >> 1);
...
bool SingleMatch2::FindGeoMatchModel(Mat srcSobelDx, Mat srcSobelDy, MatchSection matchsect, size_t nofEdgePiex,
	CvPoint *pPositionEdge, double *pEdgeMagnitude, double *pEdgeMX, double *pEdgeMY,
...
bool SingleMatch2::DoInspect(Mat InputSrcImg, Mat InputTmpImg, vector<MatchRst> &RstVec, int nMatchNum, double dScoreThreshold)
{
...
bool SingleMatch2::Match(Mat InputSrcImg, Mat InputTmpImg, double dScoreThreshold, int nPyrCont,
	MatchSection matchsect, vector<MatchRst> &RstVec)
{
	...
	int tmpHe = InputTmpImg.rows;
	int tmpWi = InputTmpImg.cols;
	int srcHe = InputSrcImg.rows;
	int srcWi = InputSrcImg.cols;

	matchsect.nStartX = -tmpWi;
	matchsect.nStartY = -tmpHe;
	matchsect.nEndX = 0;
	matchsect.nEndY = 0;
	...
	do {
		//块划分
		matchsect.nStartY += tmpHe;
		matchsect.nEndY += tmpHe;
		if (matchsect.nEndY > srcHe)
		{
			matchsect.nEndY = srcHe;
			matchsect.nStartY = srcHe - tmpHe;
		}
		do 
		{
			matchsect.nStartX += tmpWi;
			matchsect.nEndX += tmpWi;
			if (matchsect.nEndX > srcWi)
			{
				matchsect.nEndX = srcWi;
				matchsect.nStartX = srcWi - tmpWi;
			}
		...
		} while (matchsect.nEndX != srcWi);
	} while (matchsect.nEndY != srcHe);
...
bool SingleMatch2::DelNearElem(vector<MatchRst> &RstVec, int nDotDist)
{
	if (RstVec.size() < 1)
	{
		return false;
